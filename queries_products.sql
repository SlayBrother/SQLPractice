-- INSERT INTO products (name, price, can_be_returned) 
-- 1 . VALUES('chair', 44.00, 'f');
-- 2 . VALUES('stool', 25.99, 't'); 
-- 3 . VALUES('table', 124.00, 'f');  
-- 4 . SELECT * FROM products;
-- 5. SELECT name FROM Products; 
-- 6. SELECT name, price FROM products; 
-- 7. INSERT INTO products (name, price, can_be_returned) VALUES ('Colby', 12000.00, 'f'); 
-- 8. SELECT * FROM products WHERE can_be_returned = 't' 
-- 9.  SELECT name FROM products WHERE price < 44.00; 
-- 10. SELECT name FROM products WHERE price > 22.50 AND price < 99.99; 
-- 11. UPDATE products SET price = price - 20;  
-- 12. DELETE FROM products WHERE price < 25;
-- 13. UPDATE products SET price = price + 20; 
-- 14. UPDATE products SET can_be_returned = 't' 
-- 15. 