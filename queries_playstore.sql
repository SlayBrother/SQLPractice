...
-- 8. FROM analytics WHERE min_installs < 50 AND reviews > 0 ORDER BY rating DESC; 
-- 9. SELECT app_name FROM analytics WHERE rating < 3 AND reviews > 10000; 
-- 10. SELECT * FROM analytics WHERE price >= .10 AND price =< 1.00 ORDER BY reviews LIMIT 10; 
-- 11. SELECT * FROM analytics WHERE last_updated = (SELECT MIN(last_updated) FROM analytics);
-- 12. SELECT * FROM analytics WHERE price = (SELECT MAX(price) FROM analytics);
-- 13. SELECT SUM(reviews) AS sum_reviews FROM analytics; 
